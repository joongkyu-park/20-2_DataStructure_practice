...
public:
	...
	// 정점삽입
	void insert_vertex(int index, string data) {
		vertex* v = new vertex(index, data);
	...
	void insert_edge(int num1, int num2, string edge_data) {
		...
		for (int i = 0; i < this->vertices_list.size();i++) {
			if (this->vertices_list[i]->index == num1) {
				v = this->vertices_list[i];
			}
			if (this->vertices_list[i]->index == num2) {
	...
	void deleteVertex(int number) {
		...
		int flag = 1;
		for (int i = 0; i < this->vertices_list.size();i++) {
			if (number == this->vertices_list[i]->index) {
				v = this->vertices_list[i];
				flag = 0;
				break;
			}
...
void DFS(graph G, int number) {
	...
	for (int i = 0; i < G.vertices_list.size(); i++) {
		if (G.vertices_list[i]->index == number) {
	...
	cout << start_v->data;
	for (int i = 0; i < G.adjacency_matrix[start_v->index].size();i++) {
		if (G.adjacency_matrix[start_v->index][i] != nullptr) {
			edge* e = G.adjacency_matrix[start_v->index][i];
